...
public class TileEntityMelter : TileEntityMachine
{
    ...
    private const string DefaultMachineRecipeGroup = "melter";
    ...
    public TileEntityMelter(Chunk chunk) : base(chunk)
    {
    }
    ...
    private void DevLog(string msg)
    {
        ...
        Log.Out($"[Melter][TE][{ToWorldPos()}] {msg}");
    }

    public override TileEntityType GetTileEntityType()
    {
        return unchecked((TileEntityType)UCTileEntityIDs.Melter);
    }
    ...
    {
        if (!TryGetConversionRule(SelectedRecipeKey, out FuelConversionRule rule) || rule == null)
            return 0;
    ...
    private void EnsureConfigLoaded()
    {
        ...
        for (int i = 0; i < machineRecipes.Count; i++)
        {
            ...
            if (!TryReadMachineRecipeAsMelterRule(
                    machineRecipe,
    ...
    public static bool TryReadMachineRecipeAsMelterRule(
        ...
        out string error)
    {
        ...
        {
            error = "Melter recipe does not support <output>";
            return false;
        }
    ...
    public static bool IsMelterRecipeAllowedForMachineGroups(MachineRecipe recipe, string machineGroupsCsv)
    {
    ...
    private string GetFirstMissingRequirementReason(WorldBase world)
    {
        ...
        int requiredHeat = GetSelectedRuleRequiredHeat();
        if (requiredHeat > 0 && CurrentHeat < requiredHeat)
            return $"Insufficient Heat ({CurrentHeat}/{requiredHeat})";
