...
{
    public TileEntityUniversalExtractor te;
    private Vector3i blockPos;

    public XUiC_ExtractorOutputContainerEntry[] entries;
    public XUiC_ExtractorOutputContainerEntry SelectedEntry;

    public void SetContext(TileEntityUniversalExtractor te, Vector3i pos)
    {
        this.te = te;
        this.blockPos = pos;
        IsDirty = true;
    ...
    public override void Init()
    {
        ...
        entries = GetChildrenByType<XUiC_ExtractorOutputContainerEntry>();
    ...
    {
        var entry = sender as XUiC_ExtractorOutputContainerEntry;
        if (entry == null || entry.ContainerTE == null || entry.OutputTarget == null)
            return;

        OutputTargetInfo target = entry.OutputTarget;

        Log.Out($"[Extractor][UI] Request output select: pos={target.BlockPos} mode={target.TransportMode} pipeGraphId={target.PipeGraphId}");

        Helper.RequestExtractorSelectOutput(
            blockPos,
            target.BlockPos,
