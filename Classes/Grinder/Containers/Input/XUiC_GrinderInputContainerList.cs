...
public class XUiC_GrinderInputContainerList : XUiController
{
    public TileEntityUniversalGrinder te;
    private Vector3i blockPos;
    public XUiC_GrinderInputContainerEntry[] entries;

    public void SetContext(TileEntityUniversalGrinder grinder, Vector3i pos)
    {
        te = grinder;
        blockPos = pos;
        IsDirty = true;
    ...
    {
        base.Init();
        entries = GetChildrenByType<XUiC_GrinderInputContainerEntry>();
        if (entries != null)
        {
    ...
    {
        XUiC_GrinderInputContainerEntry entry = sender as XUiC_GrinderInputContainerEntry;
        if (entry == null || entry.ContainerPos == Vector3i.zero)
            return;

        Helper.RequestGrinderSelectInput(blockPos, entry.ContainerPos, entry.PipeGraphId.ToString());
        IsDirty = true;
