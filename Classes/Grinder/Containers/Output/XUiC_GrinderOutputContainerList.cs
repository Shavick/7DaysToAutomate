...
public class XUiC_GrinderOutputContainerList : XUiController
{
    public TileEntityUniversalGrinder te;
    private Vector3i blockPos;
    public XUiC_GrinderOutputContainerEntry[] entries;
    ...
    {
        base.Init();
        entries = GetChildrenByType<XUiC_GrinderOutputContainerEntry>();
        if (entries != null)
        {
    ...
    {
        XUiC_GrinderOutputContainerEntry entry = sender as XUiC_GrinderOutputContainerEntry;
        if (entry?.OutputTarget == null)
            return;

        OutputTargetInfo target = entry.OutputTarget;
        Helper.RequestGrinderSelectOutput(blockPos, target.BlockPos, (int)target.TransportMode, target.PipeGraphId.ToString());
        IsDirty = true;
