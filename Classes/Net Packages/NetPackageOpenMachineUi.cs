        ...
        public override void ProcessPackage(World _world, GameManager _callbacks)
        {
            ...
            if (!ConnectionManager.Instance.IsServer && type == MessageType.OpenClient)
            {
                ...
                switch (CustomUi)
                {
                    ...
                    case "FluidMixerInfo":
                        if (!(te is TileEntityFluidMixer))
                        {
                            Log.Error($"[NetPkg][MachineUI][CLIENT] TileEntity is not TileEntityFluidMixer for UI {CustomUi}");
                            return;
                        }

                        Log.Out($"[NetPkg][MachineUI][CLIENT] Opening Fluid Mixer UI at {BlockPos}");
                        localPlayer.AimingGun = false;
                        XUiC_FluidMixerInfo.Open(localPlayer, BlockPos);
                        Log.Out($"[NetPkg][MachineUI][CLIENT] Fluid Mixer UI open call executed");
                        break;

                    case "BoilerInfo":
                        if (!(te is TileEntityBoiler))
                        {
                            Log.Error($"[NetPkg][MachineUI][CLIENT] TileEntity is not TileEntityBoiler for UI {CustomUi}");
                            return;
                        }

                        Log.Out($"[NetPkg][MachineUI][CLIENT] Opening Boiler UI at {BlockPos}");
                        localPlayer.AimingGun = false;
                        XUiC_BoilerInfo.Open(localPlayer, BlockPos);
                        Log.Out($"[NetPkg][MachineUI][CLIENT] Boiler UI open call executed");
                        break;

                    case "CasterInfo":
                        if (!(te is TileEntityCaster))
                        {
                            Log.Error($"[NetPkg][MachineUI][CLIENT] TileEntity is not TileEntityCaster for UI {CustomUi}");
                            return;
                        }

                        Log.Out($"[NetPkg][MachineUI][CLIENT] Opening Caster UI at {BlockPos}");
                        localPlayer.AimingGun = false;
